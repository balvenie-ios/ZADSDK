...
由於本 SDK 內建獨立的 `UIWindow` 管理機制，SwiftUI 開發者可以直接呼叫 API，無需處理 `UIViewControllerRepresentable`。以下提供三種常見整合情境：

### 情境 1：App 啟動後立即顯示
若希望 App 一啟動就顯示廣告，建議在 `WindowGroup` 的內容載入後呼叫。為了確保主視窗（KeyWindow）已完全準備好，建議加上些微延遲。
...
### 情境 2：進入特定畫面時顯示
利用 SwiftUI 的 `.onAppear` 修飾符，可以在使用者進入該頁面時自動觸發廣告。
...
### 情境 3：點擊按鈕後顯示
這是最常見的互動模式，直接在 Button 的 Action Closure 中呼叫即可。
